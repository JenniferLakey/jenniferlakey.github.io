...
void ShapeMeshes::LoadCurvedConeMesh(int numSlices, int curveSteps, float radius, float height, float bendRadius)
{
	...
	for (int step = 0; step <= curveSteps; ++step) {
		float t = static_cast<float>(step) / curveSteps;
		float arcTheta = t * bendAngle;
		...
		for (int slice = 0; slice <= numSlices; ++slice) {
			...
			// Approximate normal (optional: improve later)
	...
	// Use your centralized mesh initializer
	...
	float sweepAngleRadians)
{
	...
	for (int i = 0; i <= mainSegments; ++i) {
		float theta = i * mainStep;
		float sweepT = static_cast<float>(i) / mainSegments;
		float tubeRadius = glm::mix(tubeRadiusStart, tubeRadiusEnd, sweepT);
...
void ShapeMeshes::DrawSpiralMesh(
	...
	// Final ring connects to tube start
	for (int j = 0; j < capSegments; ++j) {
...
void ShapeMeshes::DrawSineConeMesh(
	...
	for (int i = 0; i <= heightSegments; ++i) {
		...
		float t = static_cast<float>(i) / heightSegments;
		...
		for (int j = 0; j <= radialSegments; ++j) {
			...
			normals.push_back(glm::vec3(0.0f)); // Placeholder
		}
	}
	...
	// Normalize accumulated normals and build final vertex buffer
